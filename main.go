...
// func webListener(url string, s *discordgo.Session, m *discordgo.MessageCreate) {
// 	for {
// 		request, err := http.NewRequest("GET", url, nil)
// 		if err != nil {
// 			fmt.Println(redColor+"Error:", err, defaultColor)
...
// 		if res.StatusCode != http.StatusOK {
// 			fmt.Printf(redColor+"Status Error : %d%s\n", res.StatusCode, defaultColor)
// 			time.Sleep(timeInterval * time.Second)
// 			continue
// 		}
...
// 		// **Updated Loop**: Scan the entire HTML document
// 		found := false
// 		for {
// 			tokenType := doc.Next()
...
// 					response := fmt.Sprintf("Hello <@%s>! Your observed item is now available!", m.Author.ID)
// 					s.ChannelMessageSend(m.ChannelID, response)
// 					fmt.Printf("%s[%d] Item is in stock!%s\n", greenColor, res.StatusCode, defaultColor)
// 					found = true
// 					break
// 				}
// 			}
...
// 		// If item was found, stop the loop
// 		if found {
// 			return
// 		}

// 		fmt.Printf("%s[%d]%s No item in stock. Refetching in %d sec...%s\n", greenColor, res.StatusCode, redColor, timeInterval, defaultColor)
// 		time.Sleep(timeInterval * time.Second)
// 	}
// }
